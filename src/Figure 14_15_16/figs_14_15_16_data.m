...
%-------------------------------------------------------------------------%
% Inputs:
% fig_num - figure number (either 14, 15 or 16)
% row_num - row number (either 1 or 2)
% col_num - column number (either 1 or 2)
%-------------------------------------------------------------------------%

function figs_14_15_16_data(fig_num,row_num,col_num)
...
for l = 1:num_cases
    ...
    % QR-Legendre, uniform, unweighted
    if l == 1
        samp_type      = 'uniform_grid';                   % type of sampling
        basis_type     = 'orthogonal';                     % functions used in the measurement matrix A
        err_basis_type = basis_type;                       % polynomials used in computing the error
        I              = reorder_index_set(I,[]);          % Reorder index set
        weighted_opt   = 0;                                % weighted opt
        
        % QR-Legendre, optimal, unweighted
    elseif l == 2
        samp_type      = 'optimal';
    ...
    for i = 1:num_m
        ...
        parfor t = 1:num_trials
            ...
            disp(['Figure ',num2str(fig_num),'_',num2str(row_num),'_',num2str(col_num),space,'dimension = ',num2str(d),space,...
                'polynomials = ',basis_type,space,'sampling = ',samp_type,space,'weighted = ',num2str(weighted_opt),space,...
                'm = ',num2str(m),space,'trial = ',num2str(t),space,'iter = ',num2str(i),space,'L2 error = ', num2str(L2_err),...
                space,'Linf error = ',num2str(Linf_err)]);
...
%%% Save data %%%
clear A_err_grid Q_err_grid R b_err_grid A b eta W w opt_mu opt_w theta err_grid

save(['../../data/Figure 14_15_16/',fig_name])
