...
% Functions", SIAM, 2021.
%-------------------------------------------------------------------------%
% Description: generates a measurement matrix based on polynomial type 
% 'lengendre', 'chevushev', 'preconditioned' or 'orthogonal' wtr an index 
% set I, over a grid y_grid.
%-------------------------------------------------------------------------%
% Inputs:
% poly_type - either 'legendre' (Legendre polynomials), 'chebyshev'
% (Chebyshev polynomials), 'preconditioned' (preconditioned Legendre
% polynomials) or 'orthogonal' (QR-decomposition of Legendre polynomials)
...
for i = 1:m
    ...
    if isequal(poly_type,'legendre') || isequal(poly_type,'preconditioned') || isequal(poly_type,'orthogonal')
        L = legmat(y',n+1);
